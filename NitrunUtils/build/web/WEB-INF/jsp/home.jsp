                        ...
                        <c:forEach items="${datos}" var="dato">
                            <tr>
                                ...
                                <td class="text-center">                                    
                                        ...
                                        <form:select path="">
                                            <form:options items="${usuarios.fileMetaList}"></form:options>
                                        </form:select>
                                    
                                    <select style="font-family: 'FontAwesome', Helvetica;">
                                        <c:forEach items="${usuarios.fileMetaList}" var="datoF" varStatus="status">
                                            <option>&#xf083; ${datoF.fileName} ${status.count} ${fileMeta.fileName}</option>
                                            <input type="checkbox" name="fileMetaList(${status.index}).fileName" value="${datoF.fileName}"> <c:out value="${datoF.fileName}" /><br>                                            
                                        </c:forEach>
                                    </select>                                    
                                    
                                    <a href="<c:out value="document.htm?idUsuario=${dato.idUsuario}"/>">
